...
export default function RecipeCard(props) {
	...
	return (
		<Card className={classes.card + " " + props.className} id={props.id}>
			...
			/>
			<CardMedia className={classes.media} image={props.recipe.imageUrl} title={props.recipe.name} />
			<CardActions disableSpacing style={{ flexWrap: "wrap" }}>
				<IconButton aria-label='add to favorites'>
					{props.recipe.favorited === 1 ? (
						<FavoriteIcon color='error' onClick={handleFavorite} />
					) : (
						<FavoriteIcon color='red' onClick={handleFavorite} />
					)}
				</IconButton>
