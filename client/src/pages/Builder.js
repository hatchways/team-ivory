...
class BuilderPage extends Component {
  ...
  removeIngredient = (targetIngredient) => {
    this.setState({
      ingredients: this.state.ingredients.filter((ingredient)=>{return ingredient != targetIngredient})
    })
  }

  removeStep = (targetStep) => {
    this.setState({
      steps: this.state.steps.filter((step)=>{return step != targetStep})
    })
  }

  removeTag = (targetTag) => {
    this.setState({
      tags: this.state.tags.filter((tag)=>{return tag != targetTag})
    })
  }
            ...
            >
              {this.state.ingredients.map((ingredient, index)=>(
                <div className={classes.ingredient} key={index}>
                  <span>{ingredient.quantity} {ingredient.unit.label}{ingredient.quantity>1&&'s'} of {ingredient.ingredient.label}</span>
                  <IconButton 
                    aria-label="remove ingredient"
              ...
              <div  
                className={classes.adderContainer+' adderContainer'}
              >
                  <div  style={{width: '20%'}}>
